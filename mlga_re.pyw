# pylint: disable=C0321,C0114,W0511,W0611
import ast
import datetime
import html
import json
...
from customException import PrintException
...
def mainLoop():
    try:
        with open(log_path, "r", encoding="utf-8") as f:
            print("File opening Succeeded! Enjoy")
            try:
                ...
                for i, line in enumerate(searchlines):
                    ...
                    if lastSteamId != 0 and (lastSteamId is not None) and any([v in line for v in killer_list.values()]):
                        ...
                        # print(type(killerSteam))
                        # print(str(killerSteam))
                        # if killerSteam != 0 or killerSteam is not None:
                        try:
                ...
                label1.config(text="Killer Steam Name, ID:  " +
                              str(killerSteamName) + " " + str(lastSteamIdParsed))
                label2.config(text="Playing Character: " +
                              str(lastCharSeen), fg="red")
